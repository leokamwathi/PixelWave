...
export var effectTextureFill := [Texture]
export var effectBeam := [Texture]
...
func _physics_process(delta):
	if playerNode:
		if !random:
			position = playerNode.global_position #get_global_mouse_position()
		if (neon_texture):
...
func expandMode(type,spin=false,spinrate=0):
	...
	if indexOfType >= 0:
		var baseScale = float(playerNode.scale.x/100)*2.0 + 0.02
		...
		setNeonTextureGrowrate(float(playerNode.scale.x*0.3) + 0.1)
...
func explodeMode(type,spin=false,spinrate=0):
	...
	if indexOfType >= 0:
		...
		setNeonTextureScale(Vector2(baseScale,baseScale))
		setNeonTextureRotation(deg2rad(0))
		setNeonTextureGrowrate(2.0 + float(playerNode.scale.x*0.3) + 0.1)
...
func randomMode(type,spin=false,spinrate=0):
	...
	var indexOfType = effectNames.find(type)
	if indexOfType >= 0:
		position.x = rand_range(0,1366)
		position.y = rand_range(0,768)
